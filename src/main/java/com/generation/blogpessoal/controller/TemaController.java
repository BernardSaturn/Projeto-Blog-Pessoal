...
public class TemaController {
	...
	public ResponseEntity<List<Tema>> getByDescricao(@PathVariable String descricao) { // Recebe uma descrição como
																						// parâmetro
		return ResponseEntity.ok(temaRepository.findAllByDescricaoContainingIgnoreCase(descricao)); // Retorna temas que
																									// contêm a
																									// descrição
																									// fornecida,
																									// ignorando
	...
	public ResponseEntity<Tema> put(@Valid @RequestBody Tema tema) { // Recebe um objeto Tema no corpo da requisição
		return temaRepository.findById(tema.getId()) // Procura o tema pelo ID
				.map(resposta -> ResponseEntity.status(HttpStatus.CREATED) // Se encontrar, retorna o tema atualizado
